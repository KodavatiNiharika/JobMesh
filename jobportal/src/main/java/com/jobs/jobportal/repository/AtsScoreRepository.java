...
public interface AtsScoreRepository extends JpaRepository<AtsScore, Long> {
    ...
    @Query("SELECT a.jobId FROM AtsScore a WHERE a.email = :email AND a.atsScore >= :threshold")
List<Long> findJobIdsWithHighAts(@Param("email") String email, @Param("threshold") int threshold);
